...
public class XiuGaiMiMaDialog extends Dialog {
    private TextView jiudianname;
    private Button l1,l2;
    private EditText idid,er;
    public XiuGaiMiMaDialog(Context context) {
        super(context, R.style.dialog_style2);
        setCustomDialog();
    }

    private void setCustomDialog() {
        View mView = LayoutInflater.from(getContext()).inflate(R.layout.xiugaidialog_mima, null);

        jiudianname=  mView.findViewById(R.id.title2);
        idid= mView.findViewById(R.id.idid);
        er=  mView.findViewById(R.id.xiangce);
    ...
    public String getUrl2(){
        return er.getText().toString().trim();
    }
